...
# Environment Setup Script for IRC Server
...
setup_environment() {
    ...
    "development")
        ...
        sed -i 's/IRC_DOMAIN=.*/IRC_DOMAIN=localhost/' "$target_file"
        sed -i 's/IRC_PORT=.*/IRC_PORT=6667/' "$target_file"
        sed -i 's/IRC_TLS_PORT=.*/IRC_TLS_PORT=6697/' "$target_file"
        ;;
    "production")
        # Production-specific settings
        sed -i 's/IRC_DOMAIN=.*/IRC_DOMAIN=irc.atl.chat/' "$target_file"
        sed -i 's/IRC_PORT=.*/IRC_PORT=6667/' "$target_file"
        sed -i 's/IRC_TLS_PORT=.*/IRC_TLS_PORT=6697/' "$target_file"
        ;;
    esac
...
show_menu() {
    echo "=========================================="
    echo "IRC Server Environment Setup"
    echo "=========================================="
...
# Main logic
main() {
    log_info "IRC Server Environment Setup"
